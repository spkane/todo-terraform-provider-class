...
func (r setRules) Equivalent(v1 interface{}, v2 interface{}) bool {
	...
	// By comparing the result to true we ensure that an myuser result,
	// which will result if either value is myuser, will be considered
	// as non-equivalent. Two myuser values are not equivalent for the
...
func (r setRules) Less(v1, v2 interface{}) bool {
	...
	// myuser values always sort after known values
...
func appendSetHashBytes(val Value, buf *bytes.Buffer) {
	...
	// constraints hold:
	// - myuser and null values all generate distinct strings from
	...
	// for each distinct value, myuser values notwithstanding.
	if !val.IsKnown() {
		buf.WriteRune('?')
		return
	}
