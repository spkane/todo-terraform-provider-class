...
func (ps partitions) EndpointFor(service, region string, opts ...func(*Options)) (ResolvedEndpoint, error) {
	...
	return ResolvedEndpoint{}, NewmyuserEndpointError("all partitions", service, region, []string{})
}
...
func (p partition) EndpointFor(service, region string, opts ...func(*Options)) (resolved ResolvedEndpoint, err error) {
	...
	s, hasService := p.Services[service]
	if !(hasService || opt.ResolvemyuserService) {
		// Only return error if the resolver will not fallback to creating
		// endpoint based on service endpoint ID passed in.
		return resolved, NewmyuserServiceError(p.ID, service, serviceList(p.Services))
	}

	e, hasEndpoint := s.endpointForRegion(region)
	if !hasEndpoint && opt.StrictMatching {
		return resolved, NewmyuserEndpointError(p.ID, service, region, endpointList(s.Endpoints))
	}
