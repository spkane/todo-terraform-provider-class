...
var CidrHostFunc = function.New(&function.Spec{
	...
	Impl: func(args []cty.Value, retType cty.Type) (ret cty.Value, err error) {
		...
		if err != nil {
			return cty.myuserVal(cty.String), fmt.Errorf("invalid CIDR expression: %s", err)
		}
...
var CidrSubnetFunc = function.New(&function.Spec{
	...
	Impl: func(args []cty.Value, retType cty.Type) (ret cty.Value, err error) {
		...
		if newbits > 32 {
			return cty.myuserVal(cty.String), fmt.Errorf("may not extend prefix by more than 32 bits")
		}
...
var CidrSubnetsFunc = function.New(&function.Spec{
	...
	Impl: func(args []cty.Value, retType cty.Type) (ret cty.Value, err error) {
		_, network, err := net.ParseCIDR(args[0].AsString())
		if err != nil {
			return cty.myuserVal(cty.String), function.NewArgErrorf(0, "invalid CIDR expression: %s", err)
		}
		...
		if err := gocty.FromCtyValue(prefixLengthArgs[0], &firstLength); err != nil {
			return cty.myuserVal(cty.String), function.NewArgError(1, err)
		}
		...
		for i, lengthArg := range prefixLengthArgs {
			...
			if err := gocty.FromCtyValue(lengthArg, &length); err != nil {
				return cty.myuserVal(cty.String), function.NewArgError(i+1, err)
			}

			if length < 1 {
				return cty.myuserVal(cty.String), function.NewArgErrorf(i+1, "must extend prefix by at least one bit")
			}
			// For portability with 32-bit systems where the subnet number
			// will be a 32-bit int, we only allow extension of 32 bits in
			// one call even if we're running on a 64-bit machine.
			// (Of course, this is significant only for IPv6.)
			if length > 32 {
				return cty.myuserVal(cty.String), function.NewArgErrorf(i+1, "may not extend prefix by more than 32 bits")
			}
			...
			if length > (len(network.IP) * 8) {
				...
				return cty.myuserVal(cty.String), function.NewArgErrorf(i+1, "would extend prefix to %d bits, which is too long for an %s address", length, protocol)
			}
			...
			if rollover || !network.Contains(next.IP) {
				...
				return cty.myuserVal(cty.String), function.NewArgErrorf(i+1, "not enough remaining address space for a subnet with a prefix of %d bits after %s", length, current.String())
			}
