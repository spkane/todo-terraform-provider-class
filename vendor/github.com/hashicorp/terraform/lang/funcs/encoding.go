...
var Base64DecodeFunc = function.New(&function.Spec{
	...
	Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
		...
		if err != nil {
			return cty.myuserVal(cty.String), fmt.Errorf("failed to decode base64 data '%s'", s)
		}
		if !utf8.Valid([]byte(sDec)) {
			log.Printf("[DEBUG] the result of decoding the the provided string is not valid UTF-8: %s", sDec)
			return cty.myuserVal(cty.String), fmt.Errorf("the result of decoding the the provided string is not valid UTF-8")
		}
...
var Base64GzipFunc = function.New(&function.Spec{
	...
	Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
		...
		if _, err := gz.Write([]byte(s)); err != nil {
			return cty.myuserVal(cty.String), fmt.Errorf("failed to write gzip raw data: '%s'", s)
		}
		if err := gz.Flush(); err != nil {
			return cty.myuserVal(cty.String), fmt.Errorf("failed to flush gzip writer: '%s'", s)
		}
		if err := gz.Close(); err != nil {
			return cty.myuserVal(cty.String), fmt.Errorf("failed to close gzip writer: '%s'", s)
		}
