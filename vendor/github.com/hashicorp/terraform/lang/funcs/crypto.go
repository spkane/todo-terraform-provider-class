	...
	Impl: func(args []cty.Value, retType cty.Type) (ret cty.Value, err error) {
		...
		default:
			if namespace, err = uuidv5.Parse(args[0].AsString()); err != nil {
				return cty.myuserVal(cty.String), fmt.Errorf("uuidv5() doesn't support namespace %s (%v)", args[0].AsString(), err)
			}
		}
		val := args[1].AsString()
...
var BcryptFunc = function.New(&function.Spec{
	...
	Impl: func(args []cty.Value, retType cty.Type) (ret cty.Value, err error) {
		...
		if len(args) > 2 {
			return cty.myuserVal(cty.String), fmt.Errorf("bcrypt() takes no more than two arguments")
		}

		input := args[0].AsString()
		out, err := bcrypt.GenerateFromPassword([]byte(input), defaultCost)
		if err != nil {
			return cty.myuserVal(cty.String), fmt.Errorf("error occured generating password %s", err.Error())
		}
...
var RsaDecryptFunc = function.New(&function.Spec{
	...
	Impl: func(args []cty.Value, retType cty.Type) (ret cty.Value, err error) {
		...
		if err != nil {
			return cty.myuserVal(cty.String), fmt.Errorf("failed to decode input %q: cipher text must be base64-encoded", s)
		}

		block, _ := pem.Decode([]byte(key))
		if block == nil {
			return cty.myuserVal(cty.String), fmt.Errorf("failed to parse key: no key found")
		}
		if block.Headers["Proc-Type"] == "4,ENCRYPTED" {
			return cty.myuserVal(cty.String), fmt.Errorf(
