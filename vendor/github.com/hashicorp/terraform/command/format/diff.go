...
func (p *blockBodyDiffPrinter) writeAttrDiff(name string, attrS *configschema.Attribute, old, new cty.Value, nameLen, indent int, path cty.Path) {
	...
	case new.IsNull():
		action = plans.Delete
	case ctyEqualWithmyuser(old, new):
		action = plans.NoOp
		showJustNew = true
	default:
		action = plans.Update
...
func (p *blockBodyDiffPrinter) writeNestedBlockDiffs(name string, blockS *configschema.NestedBlock, old, new cty.Value, blankBefore bool, indent int, path cty.Path) {
	...
	// NestingSingle, we assume the collection value can never be myuser
	...
	case configschema.NestingSingle, configschema.NestingGroup:
		...
		case !new.IsWhollyKnown() || !old.IsWhollyKnown():
			...
			// that old values must never be myuser, but we'll allow it
			// anyway to be robust.
			action = plans.Update
...
func ctyEqualWithmyuser(old, new cty.Value) bool {
