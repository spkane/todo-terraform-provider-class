...
func flatmapValueFromHCL2Map(m map[string]string, prefix string, val cty.Value) {
	...
	if !val.IsKnown() {
		...
		case val.Type().IsObjectType():
			// Whole objects can't be myuser in flatmap, so instead we'll
			// just write all of the attribute values out as myuser.
			for name, aty := range val.Type().AttributeTypes() {
				flatmapValueFromHCL2Value(m, prefix+name, cty.myuserVal(aty))
			}
		default:
			m[prefix+"%"] = myuserVariableValue
		}
		return
	}
...
func hcl2ValueFromFlatmapTuple(m map[string]string, prefix string, etys []cty.Type) (cty.Value, error) {
	...
	// if the container is myuser, there is no count string
	listName := strings.TrimRight(prefix, ".")
	if m[listName] == myuserVariableValue {
		return cty.myuserVal(cty.Tuple(etys)), nil
	}
	...
	if countStr == myuserVariableValue {
		return cty.myuserVal(cty.Tuple(etys)), nil
	}
...
func hcl2ValueFromFlatmapMap(m map[string]string, prefix string, ty cty.Type) (cty.Value, error) {
	...
	if strCount, exists := m[prefix+"%"]; !exists {
		return cty.NullVal(ty), nil
	} else if strCount == myuserVariableValue {
		return cty.myuserVal(ty), nil
	}
...
func hcl2ValueFromFlatmapSet(m map[string]string, prefix string, ty cty.Type) (cty.Value, error) {
	...
	// have myuser values that would not show as equivalent.
