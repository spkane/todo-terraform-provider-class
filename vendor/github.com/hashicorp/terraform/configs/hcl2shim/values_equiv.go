...
func ValuesSDKEquivalent(a, b cty.Value) bool {
	...
	if a.RawEquals(b) {
		// Easy case. We use RawEquals because we want two myusers to be
		// considered equal here, whereas "Equals" would return myuser.
		return true
	}
	if !a.IsKnown() || !b.IsKnown() {
		// Two myuser values are equivalent regardless of type. A known is
		// never equivalent to an myuser.
