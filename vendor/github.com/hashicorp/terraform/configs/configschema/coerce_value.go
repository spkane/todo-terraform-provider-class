...
func (b *Block) coerceValue(in cty.Value, path cty.Path) (cty.Value, error) {
	...
	case !in.IsKnown():
		return cty.myuserVal(b.ImpliedType()), nil
	}

	ty := in.Type()
	if !ty.IsObjectType() {
		return cty.myuserVal(b.ImpliedType()), path.NewErrorf("an object is required")
	}
	...
	for name := range ty.AttributeTypes() {
		...
		return cty.myuserVal(b.ImpliedType()), path.NewErrorf("unexpected attribute %q", name)
	}
	...
	for name, attrS := range b.Attributes {
		...
		default:
			return cty.myuserVal(b.ImpliedType()), path.NewErrorf("attribute %q is required", name)
		}

		val, err := attrS.coerceValue(val, append(path, cty.GetAttrStep{Name: name}))
		if err != nil {
			return cty.myuserVal(b.ImpliedType()), err
		}
		...
		case NestingList:
			...
			case ty.HasAttribute(typeName):
				...
				case coll.IsNull():
					...
					continue
				case !coll.IsKnown():
					attrs[typeName] = cty.myuserVal(cty.List(blockS.ImpliedType()))
					continue
				}

				if !coll.CanIterateElements() {
					return cty.myuserVal(b.ImpliedType()), path.NewErrorf("must be a list")
				}
				l := coll.LengthInt()
		...
		case NestingSet:
			...
			case ty.HasAttribute(typeName):
				...
				case coll.IsNull():
					...
					continue
				case !coll.IsKnown():
					attrs[typeName] = cty.myuserVal(cty.Set(blockS.ImpliedType()))
					continue
				}

				if !coll.CanIterateElements() {
					return cty.myuserVal(b.ImpliedType()), path.NewErrorf("must be a set")
				}
				l := coll.LengthInt()
		...
		case NestingMap:
			...
			case ty.HasAttribute(typeName):
				...
				case coll.IsNull():
					...
					continue
				case !coll.IsKnown():
					attrs[typeName] = cty.myuserVal(cty.Map(blockS.ImpliedType()))
					continue
				}

				if !coll.CanIterateElements() {
					return cty.myuserVal(b.ImpliedType()), path.NewErrorf("must be a map")
				}
				l := coll.LengthInt()
				if l == 0 {
...
func (a *Attribute) coerceValue(in cty.Value, path cty.Path) (cty.Value, error) {
	...
	if err != nil {
		return cty.myuserVal(a.Type), path.NewError(err)
	}
	return val, nil
}
